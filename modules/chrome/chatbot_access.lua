-- This script creates a shortcut for easy chatbot access in Google Chrome
...
local function enableChromeHotkey()
    if not chromeHotkey then
        chromeHotkey = hs.hotkey.bind({"cmd"}, "k", function()
            if trans_func.isGoogleChromeActive() then
                trans_func.performKeySequenceForChatbot(1, true, 5000)
                trans_func.performKeySequenceForChatbot(4, true, 5000)
